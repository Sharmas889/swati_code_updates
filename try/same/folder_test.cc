////This is for testing folder push
...
//! time reference value (persistent)
   ...
   double dt_max = 1000.0 / unit_time_fluid;
...
// End time for interval
   double end_t = (5.0e5 - 1)  / unit_time_fluid;
...
// Max duration of the trajectory
   double maxtime = 5.0e5 / unit_time_fluid;          
...
// MOMENTUM1
  GeoVector momentum_1(1.0 * momentum1, 0.0, 0.0);
  container.Insert(momentum_1);

// MOMENTUM2
   GeoVector momentum_2(5.0 * momentum1, 0.0, 0.0);
   container.Insert(momentum_2);


   simulation->AddDistribution(DistributionMomentumPositionUniform(), container);
...
// Largest value
   GeoVector maxval3(20.0 * momentum1, 0.0, 0.0);
...
// Position maximum
  GeoVector position_2(1.0e-1, 0.0, 0.0);
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   std::string simulation_files_prefix = "distro_xsh_fix_";

   simulation->DistroFileName(simulation_files_prefix);
   simulation->SetTasks(n_traj, batch_size);
   simulation->MainLoop();
   simulation->PrintDistro1D(0, 0, simulation_files_prefix + "time_500k_dt_1k.dat", false);   
   simulation->PrintDistro1D(1, 0, simulation_files_prefix + "pos_500k_dt_1k.dat", false); 
   simulation->PrintDistro1D(2, 0, simulation_files_prefix + "mom_500k_dt_1k.dat", false); 
